...
_Last updated: 30 Aug 2026. The site is LIVE on its own domain. This file was rewritten from
scratch on that date; earlier versions described a structure that no longer exists._

---

## ⚠️ FIRST THING TO DO

**Analytics is wired but switched off, waiting on two IDs.** Both providers load from two
constants at the top of `js/investors.js` and stay dormant while those are empty strings:

```js
const CF_BEACON_TOKEN    = '';   // Cloudflare Web Analytics
const CLARITY_PROJECT_ID = '';   // Microsoft Clarity
```

- **Cloudflare Web Analytics** — dashboard → Analytics & Logs → Web Analytics → Add a site →
  use `aioniklabs.com` (the real domain, now that it exists) → copy the `token` value out of
  the snippet. 32 hex characters. Free, no cookies, no consent banner needed.
- **Microsoft Clarity** — clarity.microsoft.com → New project → site URL `aioniklabs.com` →
  copy the short alphanumeric id from the install snippet. Free, unlimited, gives heatmaps
  and session replay. NOTE: Clarity sets cookies, so a consent banner is likely needed if
  EU traffic matters. Cloudflare alone needs nothing.

Paste both, commit, push. Verified working in both states (nothing injected when blank, both
script tags injected when filled). CTA clicks and form submits already fire as Clarity custom
events, so the funnel shows up alongside the replays.
...
1. **Analytics IDs.** See the top of this file. Only real outstanding task.
2. **Consent banner** decision if Clarity stays on and EU traffic matters.
3. **Video labels on pdms.html** are generic ("Fabrication walkthrough, parts one to three")
   because the titles were never verified. Dane can supply real titles.
4. **`main-site-draft.html`** still has the placeholder address `contact@aionik.example`.
   Fix before that page is ever unparked.
5. **Optional:** carry TM onto the rebuilt main site when it exists, and move `.logo .tm` from
   `investors.css` into `styles.css` so both pages share it.
6. **Would strengthen the site if obtained:** independent third-party test data with
